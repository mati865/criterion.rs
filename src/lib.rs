...
")
        ...
        if matches.is_present("measurement-time") {
            let num_seconds = matches.value_of_t("measurement-time").unwrap_or_else(|e| {
                println!("{}", e);
                std::process::exit(1)
            });
        ...
        if matches.is_present("nresamples") {
            let num_resamples = matches.value_of_t("nresamples").unwrap_or_else(|e| {
                println!("{}", e);
                std::process::exit(1)
            });
        ...
        if matches.is_present("noise-threshold") {
            let num_noise_threshold = matches.value_of_t("noise-threshold").unwrap_or_else(|e| {
                println!("{}", e);
                std::process::exit(1)
            });
        ...
        if matches.is_present("confidence-level") {
            let num_confidence_level = matches.value_of_t("confidence-level").unwrap_or_else(|e| {
                println!("{}", e);
                std::process::exit(1)
            });
        ...
        if matches.is_present("significance-level") {
            let num_significance_level =
                matches
                    .value_of_t("significance-level")
                    .unwrap_or_else(|e| {
                        println!("{}", e);
                        std::process::exit(1)
                    });
